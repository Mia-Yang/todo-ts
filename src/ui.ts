import { postItemIntoServer } from "./request";

const todoList: HTMLElement = document.querySelector(
  ".todo-list"
) as HTMLElement;
const inputText: HTMLInputElement = document.querySelector(
  ".inputbox"
) as HTMLInputElement;
...
export function renderItem(item: todo): void {
  ...
  todoItem.innerHTML = `<input type="checkbox" onclick="toggleTodo(${item.id})" ${isChecked}> 
